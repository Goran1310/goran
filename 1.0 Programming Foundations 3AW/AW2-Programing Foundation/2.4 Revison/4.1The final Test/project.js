...
function newCustomer() {
    ...
    //THERE ARE CURRENTLY SOME CUSTOMERS
    if(customers.length > 0) {
        let tableTaken = false;
    //CHECK CURENTLY SEATED TABELS
        for( let i = customers.length-1; i>=0; i--) {
        ...
        //TABLE IS TAKEN
        if(tableTaken) {
            alert("Table " + tableNumber + "is already taken!");
            console.log(customers);
        } else {
    ...
    //TABLE IS NOT TAKEN
        ...
        console.log("No customers, all tabels available")
    alert("You got a table of number: " + tableNumber + ", which by default can fit " + tableSizes[tableNumber] + " people.")
    customers.push([name, tableNumber]);
    console.log(customers);
    }
...
// https://www.youtube.com/watch?v=8yhaePTGC6s